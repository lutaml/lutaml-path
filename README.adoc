...
Operator precedence, from tightest to loosest: `!`, then comparison
(`=`, `!=`, `>`, `<`, `>=`, `<=`), then `&&`, then `||`. Use parentheses to
override it.

Comparison values are single-quoted strings (`'author'`) or bare numbers
(`30`). Use `\'` for a literal quote inside a value.

Within a segment, `[` opens a filter when the bracket group ends the segment
and its body contains a condition operator. Otherwise `[` is an ordinary
wildcard character set, so `Class[A-Z]` remains a pattern.
